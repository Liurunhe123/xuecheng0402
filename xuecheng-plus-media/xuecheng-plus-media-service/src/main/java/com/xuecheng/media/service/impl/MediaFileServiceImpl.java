...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    public RestResponse<Boolean> checkChunk(String fileMd5, int chunkIndex) {
        //分块存储路径是，MD5的前两位为两个目录，chunk存储分块文件
    ...
    public RestResponse uploadChunk(String fileMd5, int chunk, String localChunkFilePath) {
        ...
        boolean b = addMediaFilesToMinIO(localChunkFilePath, mimeType,bucket_video, chunkFilePath+chunk);
        if (!b) {
    ...
    public RestResponse mergechunks(Long companyId, String fileMd5, int chunkTotal, UploadFileParamsDto uploadFileParamsDto) {
        ...
        ComposeObjectArgs composeObjectArgs = ComposeObjectArgs.
                builder().sources(sources).bucket(bucket_video).
                object(objectname)
                .build();
        try {
